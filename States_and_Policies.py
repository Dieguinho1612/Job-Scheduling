...
def create_all_states(list_jobs,list_machines,initial_runtimes):
    
    #initiate list of states = []
    list_states = []
    
    #create initial state
    initial_state = create_initial_state(initial_runtimes)
    ID = 0
    ...
    while current_states:
        ...
        for state in current_states:
            ...
            if sum(state.jobs_remaining) == 0:
                #add remaining costs until everything is finished
...
def create_initial_state(initial_runtimes):
    
    #create initial circumstances
    time = 0
    jobs_remaining = [1]*n
    machines_on_duty = [1]*m
    free_machines = [1 if x==0 else 0 for x in initial_runtimes]
    machine_runtimes = initial_runtimes
    machine_occupations = [None if x==0 else -1 for x in initial_runtimes] # "-1" stands for an initial occupation
...
def assign_job(state,job,machine):
    ...
    successor_state.costs = time_difference + deadline_costs(state,job,machine)
...
def turn_off_machine(state, machine):
    ...
    successor_state.costs = time_difference
...
def deadline_costs(state, job, machine):
    
    proc_time = job.processing_time[machine.index]
    st = state.time
    et = st + proc_time
    #job deadline overdue costs
    job_deadline_costs = max(0,job.weight*(et - job.deadline))
    machine_deadline_costs = max(0,machine.weight*(et - max(machine.deadline,st)))
    
    return job_deadline_costs + machine_deadline_costs
...
def state_to_data(state):
    ...
    #merge and flatten them
    state.input = [jobs_data, machines_data]
    
    """
    #filter sets job entries to zero if job is not available (row) or if machine is turned off (column)
    jobs_filter = np.vstack([state.jobs_remaining]*n).T * np.vstack([state.machines_on_duty+[1,1]]*n)    
    #filter sets machine entries to zero if it was turned off already (row)
    machines_filter = np.vstack([state.machines_on_duty]*2).T
     
    #create data for state
    state_data = np.array([state.time] #time of state
                          + state.machines_on_duty #which machines are still operating
                          + state.free_machines #which machines are free right now
                          + state.machine_runtimes) #how long until the other machines finish their current job
    
    #we also need a vector to switch of the values of infeasible actions in the NN later on
    shut_down_option = min(sum(state.machines_on_duty),2) - 1 #1 if turning down machine is an option, 0 elsewise
    state_feasability = np.array(state.jobs_remaining + [shut_down_option])

    #input consists of data of jobs and machines of the probleme as well as data of the state situation
    state.input = [
        jobs_data * jobs_filter,
        machines_data * machines_filter,
        state_data,
        state_feasability
    ]"""
...
def backtracking(all_states):
    ...
    while states_to_backtrack:
        ...
        for state in temp_final_states:
            ...
            #define target vector corresponding to Q-Values and One-Hot-Vector indicating optimal decision
            if sum(state.jobs_remaining) > 0:
                target = np.array([qvalue for qvalue in state.Qvalues if qvalue])
                state.target = [target, 
                                np.eye(target.shape[0])[np.argmax(target)],
                               np.min(target)/target]
                """state.target = np.array([qvalue if qvalue else 0 for qvalue in state.Qvalues])"""
...
def compute_all_states(jobs, machines, initial_runtimes, max_r, max_d, max_w, name="all_states"):
    ...
    #create all states
    all_states = create_all_states(list_jobs,list_machines, initial_runtimes)
