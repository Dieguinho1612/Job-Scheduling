...
class states:
    def __init__(self, time, jobs_remaining, machine_information, predecessor):
        ...
        self.machine_information = machine_information
        ...
        for state in current_states:
            ...
            if sum(state.jobs_remaining) == 0:
                ...
                if state.machine_information:
                    state.costs += np.nanmax(np.array(state.machine_information, dtype=np.float64))
            ...
            #create one state for every remaining job assigned to the free machine with lowest index
                ...
                machine = list_machines[state.machine_information.index(0)]
                ...
                if sum(x is not None for x in state.machine_information) > 1:                       
...
def create_initial_state():
    ...
    time = 0
    jobs_remaining = [1]*n
    machine_information = [machine.init_runtime for machine in list_machines]
    
    #create initial state
    initial_state = states(time, jobs_remaining, machine_information, None)
...
def assign_job(state,job,machine):
    ...
    machine_information = state.machine_information.copy()
    machine_information[machine.index] = job.processing_time [machine.index]
    
    #if there is another free machine, we create a successor state at same time
    if 0 in machine_information:
        #no time passes
        time_difference = 0
        #create state
        successor_state = states(state.time, jobs_remaining, machine_information, state)
    
    #elsewise we have to proceed to the point where the next job finishes
    else:
        #calculate new time for when next job finishes
        time_difference = min([runtime for runtime in machine_information if runtime])
        new_time = state.time + time_difference
                                                
        #update machine runtimes
        for i, runtime in enumerate(machine_information):
            if runtime:
                machine_information[i] = max(runtime - time_difference,0)
                            
        #create succesor state
        successor_state = states(new_time, jobs_remaining, machine_information, state)
...
def turn_off_machine(state, machine):
    ...
    machine_information = state.machine_information.copy()
    machine_information[machine.index] = None
    ...
    if 0 in machine_information:
        ...
        time_difference = 0
        #create state
        successor_state = states(state.time, state.jobs_remaining, machine_information, state)
        
    #elsewise we have to proceed to the point where the next job finishes
    else:
        #calculate new time for when next job finishes
        time_difference = min([runtime for runtime in state.machine_information if runtime])
        new_time = state.time + time_difference
        ...
        #create succesor state
        successor_state = states(new_time, state.jobs_remaining, machine_information, state)
...
def initialize_costs(initial_state):
    ...
    costs = 0
    for i, runtime in enumerate(initial_state.machine_information):
...
def compute_all_states(jobs, machines, MVS=0, JS=0, save=False, name="all_states"):#jobs, machines, max_r, max_d, max_w, MVS=0, JS=0, save=False, name="all_states"):
    ...
    if save:
        path = f'{name}_compressed.pickle'
